...
public class MeshDestroy : MonoBehaviour
{
    ...
    public int MaxParticles = 100; // New public variable to set the max number of particles

    private int currentParticles = 0; // Tracks the current number of parts

    void OnCollisionEnter(Collision collision)
    {
        // Check if the collision is with an obstacle (you can add a tag check here if needed)
    ...
    private void DestroyMesh()
    {
        ...
        parts.Add(mainPart);
        currentParticles = 1; // Starting with the original mesh as the first part

        for (var c = 0; c < CutCascades && currentParticles < MaxParticles; c++)
        {
            for (var i = 0; i < parts.Count && currentParticles < MaxParticles; i++)
            {
                ...
                // If the limit is reached, break early
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(
                parts[i].Bounds.center * ExplodeForce,
                transform.position
            );
        }
